...
os.makedirs('../commits_per_package', exist_ok=True)
...
# package_id = package_ids_max + 1

package_id = 0

with open(f'../package_ids.csv', 'w') as f:
...
for db in tqdm(dbs, leave=True, desc="dbs", position=0):
    ...
    df = pd.DataFrame()
    try:
        df = pd.read_csv(f'../unique_commits_per_package/{id}_unique_commit_ids.csv')
    except:
        continue
    ...
    for index, row in tqdm(df.iterrows(), position=1, leave=False, desc="package"):
        ...
        #     [result_df[col].dropna() for col in ['commit_date']]).values
        #
        # })
        ...
        result_df.to_csv(f'../commits_per_package/db_{id}_{projectId}_{package_id}_commits.csv', index=False)
        package_ids += f'{package},{id},{projectId},{package_id}\n'
        package_id += 1


    with open(f'../package_ids.csv', 'a') as f:
        f.write(package_ids)
