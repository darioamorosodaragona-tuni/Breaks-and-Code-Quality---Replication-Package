...
os.makedirs('../commits_per_packager', exist_ok=True)
...
for db in tqdm(dbs, leave=True, desc="dbs", position=0):
    ...
    df = pd.DataFrame()
    try:
        df = pd.read_csv(f'../unique_commits_per_package_with_pr_id/{id}_unique_commits_ids.csv')
    ...
    for index, row in tqdm(df.iterrows(), position=1, leave=False, desc="package"):
        ...
        if not issue_df.empty:
            with open(f'../not_empty_2.csv', 'a') as f:
