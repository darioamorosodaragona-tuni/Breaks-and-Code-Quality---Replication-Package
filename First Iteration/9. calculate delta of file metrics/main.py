...
os.makedirs('../files_per_package_with_metric_and_sonar_with_delta', exist_ok=True)
...
for file in tqdm(files, position=1, leave=False, desc="Processing packages"):
    ...
    try:
        file_df = pd.read_csv(f'../files_per_package_with_metric_and_sonar/{file}')
    except:
    ...
    file_df.to_csv(f'../files_per_package_with_metric_and_sonar_with_delta/{file}', index=False)
