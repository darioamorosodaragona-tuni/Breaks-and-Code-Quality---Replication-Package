...
commits_per_package = "../unique_commits_per_package"
os.makedirs("../unique_commits_per_package_with_pr_id", exist_ok=True)
...
for file in tqdm(os.listdir(commits_per_pr), position=0, leave=True, desc="Processing commits per PR"):
    if file.endswith('.sql.csv'):
        ...
        for index, row in tqdm(df.iterrows(), position=1, leave=False, desc="Processing rows"):
            ...
            # da qui
            df_pr.to_csv(f"../unique_commits_per_package_with_pr_id/{db}_unique_commits_ids.csv", index=False)
