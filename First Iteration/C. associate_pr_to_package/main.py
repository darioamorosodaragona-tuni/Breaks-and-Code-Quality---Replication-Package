...
def update_column_A(row, commit_id, project_id, pull_request_number):
    if commit_id in row['unique_commit_ids'].split(','):
        if project_id == row['project_id']:
            return pull_request_number + ',' + str(row['prq_number']) if pd.notna(row['prq_number']) else pull_request_number
    else:
        return row['prq_number']

commits_per_pr = "../commits_per_pr"
commits_per_package = "../unique_commits_per_package_pr"
os.makedirs("../unique_commits_per_package_pr_with_pr_id", exist_ok=True)
...
for file in tqdm(os.listdir(commits_per_pr), position=0, leave=True, desc="Processing commits per PR"):
    if file.endswith('.sql.csv'):
        ...
        for index, row in tqdm(df.iterrows(), position=1, leave=False, desc="Processing rows"):
            ...
            df_pr['prq_number'] = df_pr.apply(lambda row: update_column_A(row, commit_id, project_id, pull_request_number), axis=1)


            # da qui
            df_pr.to_csv(f"../unique_commits_per_package_pr_with_pr_id/{db}_unique_commits_ids.csv", index=False)
