...
os.makedirs('../unique_commits_per_package', exist_ok=True)
...
query = f"""
    SELECT git_commit.git_user_id, git_commit.project_id, GROUP_CONCAT(DISTINCT git_commit_id) AS unique_commit_ids
    FROM {git_files}, git_commit
    WHERE {git_files}.git_commit_id = git_commit.id
    GROUP BY git_commit.git_user_id, git_commit.project_id
"""
