...
os.makedirs('../files_per_developer_with_sonar', exist_ok=True)
package_id = 0
for db in tqdm(dbs, position=0, leave=True, desc="Processing databases"):
    ...
    files = os.listdir('../commits_per_developer_concat_with_delta')
    ...
    for file in tqdm(files, position=1, leave=False, desc="Processing packages"):
        ...
        df = pd.read_csv(f'../commits_per_developer_concat_with_delta/{file}')
        ...
        for index, row in df.iterrows():
            ...
            query = f"""
                SELECT *
                FROM git_files
                WHERE git_commit_id = {commit_id}
            """

            query2 = f'''
                SELECT git_user_id, sonar_measures.id
                FROM git_commit
            ...
            if row['pr']:
                ...
                try:
                    ...
                    with open('errors.csv', 'a') as f:
                        f.write(f"{file},{db_id},{commit_id},{developer},{prq}\n")
                conn.close()
        ...
        if os.path.exists(f'../files_per_developer_with_sonar/{file}'):
            file_df.to_csv(f'../files_per_developer_with_sonar/{file}', mode="a", index=False, header=False)
        else:
            file_df.to_csv(f'../files_per_developer_with_sonar/{file}', mode="a", index=False)
