...
comm_per_package = os.listdir('../commits_per_developer_concat')

os.makedirs('../commits_per_developer_concat_with_delta', exist_ok=True)

for commits in tqdm.tqdm(comm_per_package, desc="Processing packages", position=0, leave=True):
    # Connect to SQLite database
    data = pd.read_csv(f'../commits_per_developer_concat/{commits}')
    ...
    data.to_csv(f'../commits_per_developer_concat_with_delta/{commits}', index=False)
