...
for db in tqdm(dbs, leave=True, desc="dbs", position=0):
    ...
    df = pd.DataFrame()
    try:
        df = pd.read_csv(f'../unique_commits_per_developer_with_pr_id/{id}_unique_commits_ids.csv')
    ...
    for index, row in tqdm(df.iterrows(), position=1, leave=False, desc="package"):
        ...
        issues_association = pd.concat(
            [issues_association, pd.read_csv(f"{dir}/from_pr/{id}_prs_issues.csv"),
