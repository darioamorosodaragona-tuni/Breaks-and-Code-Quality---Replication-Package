...
clusters = os.listdir("../13. Truck Factor")
...
for file in clusters:
    df = pd.read_csv(f'../13. Truck Factor/{file}')

    db_id = file.split('db-split')[1].split('clusters.csv')[0]
    db_to_connect = "../../Data/dbs/db-split" + db_id + ".sql"
    ...
    query = f"""
          ...
          LEFT JOIN git_users gu ON df.dev_name = gu.author_name AND df.project_id = gu.project_id
      """
